...
const connMySQL = mysql.createConnection(MYSQL_URI);
...
app.use((req, res, next) => {
    ...
    if (req.cookies.session) {
        jwt.verify(req.cookies.session, JWT_KEY, (err: any, decoded: any) => {
            if (err) console.error(err);
...
app.post('/admin/add-project', (req, res) => {
    connMySQL.execute('INSERT INTO projects (title, description, link, type) VALUES (?, ?, ?, ?);', [req.body.title, req.body.description, req.body.link, req.body.type], (err, rows: any, fields) => {
        if (err) console.error(err);

        req.body.techs.forEach((tech: string) => {
